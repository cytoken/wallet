...
class FileController extends AuthController
{
    ...
    public function upload(){
        try{
            ...
            if (!$info) {// 上传错误提示错误信息
                ...
                $thumbPath = str_replace('Raw', 'Raw', '.' . $localLicence);
                $this->loger("thumbPath", $thumbPath);
                //产生缩略图
                $image = new Image();
                $image->open('.' . $localLicence);
    ...
    public function uploadFile(){
        try{
            ...
            $upload->exts     = array ( 'rm', 'rmvb', 'wmv', 'avi', 'mp4', '3gp', 'mkv', 'flv');
            $upload->rootPath = './Uploads/';
            $upload->savePath = 'CompanyLicense/'; // 设置附件上传（子）目录
